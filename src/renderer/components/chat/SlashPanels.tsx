...
// ── Branch panel ────────────────────────────────────────────────
const BranchPanel = memo(function BranchPanel({ onDismiss }: { onDismiss: () => void }) {
    ...
    <PanelShell onDismiss={onDismiss}>
      ...
      <div className="flex items-center gap-2 px-3 pb-2">
        <input
          ...
          autoFocus
          className="flex-1 rounded-md border border-border/40 bg-background/50 px-2 py-1.5 text-[12px] text-foreground outline-none placeholder:text-muted-foreground focus:border-border/80"
          aria-label="Branch name"
        />
        <button
          type="button"
          disabled={!name.trim() || isCreating || !workspace}
          onMouseDown={(e) => { e.preventDefault(); void handleCreate() }}
          className="shrink-0 rounded-md bg-primary px-3 py-1.5 text-[12px] font-medium text-primary-foreground transition-colors hover:bg-primary/90 disabled:opacity-50"
...
// ── Worktree panel ──────────────────────────────────────────────
const WorktreePanel = memo(function WorktreePanel({ onDismiss }: { onDismiss: () => void }) {
  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      ...
      // Create a new task in the worktree
      ...
      store.upsertTask({
        ...
        messages: [{ role: 'system', content: `Working in worktree \`${result.worktreePath}\` on branch \`${result.branch}\``, timestamp: new Date().toISOString() }],
      })
  ...
  return (
    <PanelShell onDismiss={onDismiss}>
      ...
      <div className="flex items-center gap-2 px-3 pb-1">
        ...
        <button
          type="button"
          disabled={!isValid || isCreating || !workspace}
          onMouseDown={(e) => { e.preventDefault(); void handleCreate() }}
          className="shrink-0 rounded-md bg-primary px-3 py-1.5 text-[12px] font-medium text-primary-foreground transition-colors hover:bg-primary/90 disabled:opacity-50"
